...
"""
    ...
    def get_thought_steps(self) -> List[str]:
        ...
        return [
            "1. REVIEW & STRATEGIZE: Carefully read the ENTIRE conversation history and the FULL retrieved legal context with all sections. Identify: (a) What arguments have YOU already made (b) What the prosecutor claimed most recently (c) What the judge's feedback was. Check the current iteration number. If iteration >= 15, plan your closing argument. If iteration >= 18, prepare final closing statement.",
            "2. CONSTRUCT ARGUMENT WITH CITATIONS: Using the retrieved legal context, construct a persuasive argument as natural dialogue. MANDATORY: Cite sources using [IPC-1], [CAS-2] format for EVERY claim. Structure: (a) Direct response to prosecutor's latest point WITH CITATION (b) Your main argument with legal support AND CITATION (c) Conclusion. Be concise (3-5 sentences). If iteration >= 18, deliver your FINAL CLOSING STATEMENT with citations and indicate you rest your case."
        ]
    ...
    async def process(self, state: AgentState) -> AgentState:
        ...
        if state["thought_step"] == 1:  # Final argument step
            ...
            print(f"[LAWYER] Citations found: {citation_analysis['citation_count']} direct citations, {citation_analysis['legal_entity_count']} legal references")
            ...
            if retrieved_docs:
                ...
                print(f"[LAWYER] Hallucination check: {hallucination_check['verification_rate']} claims verified")
                if hallucination_check['warnings']:
                    for warning in hallucination_check['warnings']:
                        print(f"[LAWYER] {warning}")
